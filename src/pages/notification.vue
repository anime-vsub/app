<template>
  ...
  <div v-if="authStore.isLogged">
      ...
      <q-list>
        <transition-group name="notify">
          <q-slide-item v-for="(item) in notificationStore.items" :key="item.id" @left="onDelete($event, item)"
            @right="onDelete($event, item)" left-color="red" right-color="red">
            ...
            <q-item>
              <q-item-section>
                <q-item-label class="text-subtitle1 text-weight-normal">{{ item.name  }} <span class="text-grey">
                    đã cập nhật

                  </span> {{ item.chap }}</q-item-label>
                <q-item-label class="text-grey">{{ item.time }}</q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-img :src="item.image" width="120px" :ratio="120/81" class="rounded-sm" />
              </q-item-section>
            </q-item>
          </q-slide-item>
        </transition-group>
    ...
    <div v-else class="absolute fit text-subtitle1 flex items-center justify-center">
      <div class="text-center">
        <img src="~/assets/img_holder_empty_style1.png" width="250" class="mx-auto mb-1" />
  ...
  <div v-else class="absolute fit text-subtitle1 flex items-center justify-center">
    <div class="text-center">
      <img src="~/assets/img_holder_error_style2.png" width="250" class="mx-auto mb-1" />
...
<script lang="ts" setup>
  ...
  async function onDelete({
    ...
    notificationStore.items.splice(
      notificationStore.items.indexOf(item) >>> 0,
      1
    )
