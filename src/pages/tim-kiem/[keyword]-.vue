<template>
  <q-page-sticky position="top" class="children:w-full bg-dark-page z-10">
    ...
    Tìm kiếm: <span class="text-weight-medium">{{ route.params.keyword }}</span>
    </div>
  </q-page-sticky>
...
<div class="pt-[32px]">
  ...
  <template v-else-if="data">
    <ScreenNotFound v-if="data.items.length === 0" />
      ...
      <q-infinite-scroll v-else ref="infiniteScrollRef" @load="onLoad" :offset="250" class="px-4">
...
const { data, loading, run } = useRequest(() => TypeNormalValue("tim-kiem", route.params.keyword, 1, true), {
...
async function onLoad(_index: number, done: (stop: boolean) => void) {
  const { curPage, maxPage, items } = await  TypeNormalValue("tim-kiem", route.params.keyword, (data.value?.curPage ?? 0) + 1, true)
