  ...
  <template v-if="authStore.isLogged">
    <div class="py-15 text-center">
      <q-avatar size="80px">
        ...
        <Icon v-else icon="fluent:person-circle-20-filled" width="80" height="80" />

        <q-btn round icon="camera" class="absolute bottom-0 right-0 bg-[rgba(0,0,0,0.5)]" size="sm" />
      </q-avatar>
    </div>
    ...
    <q-list class="text-[15px]">
      ...
      <q-item clickable v-ripple @click="showDialogChangePassword = true">
        ...
        <q-item-section side>
          <div class="flex items-center flex-nowrap">
            <Icon icon="fluent:chevron-right-24-regular" width="18" height="18" class="ml-2" />
          </div>
        </q-item-section>
      </q-item>
    </q-list>
    ...
    <div class="mx-3">
      <q-btn outline color="red-5" class="w-full mt-10" @click="logout">Đăng xuất</q-btn>
    </div>
      ...
      <q-card class="h-[60vh] bg-dark-500">
        ...
        <q-card-section>
          <form @submit.prevent="changePassword">
            <div class="mb-10 relative flex items-center flex-nowrap input-wrap">
              <input v-model="new_password" required :type="showPassword ? 'text' : 'password'" name="new_password"
            ...
            <q-btn type="submit" no-caps class="text-main w-full" outline :disable="!new_password">Đổi mật khẩu</q-btn>
          </form>
        </q-card-section>
      </q-card>
    </q-dialog>
