<template>
  <header class="fixed w-full top-0 left-0 z-200 bg-transparent">
    <q-toolbar class="relative">
      ...
      <q-toolbar-title class="text-[16px] line-clamp-1">
        {{ metaPlaylist?.name }}
      </q-toolbar-title>
    </q-toolbar>
  </header>

  <div v-if="metaPlaylist && movies" class="md:row">
    <div class="md:w-[360px]">
      <div class="md:fixed md:h-[calc(100vh-58px)] md:w-[360px] py-4 <md:pt-0">
        <div
          class="relative md:h-full bg-[#594d42] px-5 py-6 overflow-hidden md:rounded-[15px]"
        >
    ...
    <div class="md:col md:pl-4 md:pt-4 mx-4">
      <q-infinite-scroll
...
const {
  ...
  },
  {
    ...
    refreshDepsAction() {
      if (!route.params.playlist) return // fix error is undefined because keep-alive
      runGetMetaPlaylist()
    },
  }
)
