<template>
  ...
  <div class="mt-4">
    <router-link class="text-subtitle1 text-weight-normal px-4 py-1 relative flex items-center justify-between"
    ...
    <div v-if="loadingHistories" class="h-[146px] mx-4 overflow-x-hidden whitespace-nowrap">
      <q-card v-for="item in 12" :key="item" class="bg-transparent inline-block history-item mr-2"
    ...
    <div v-else-if="!errorHistories" class="mx-4 overflow-x-auto whitespace-nowrap">
      <q-card v-for="item in histories" :key="item.id" class="bg-transparent inline-block history-item mr-2"
        style="white-space: initial" @click="router.push(`/phim/${item.id}/${item.last.chap}`)">
  ...
  <div class="mt-4">
    ...
    <div v-if="loadingFavorites" class="mx-4 overflow-x-hidden whitespace-nowrap">
      <SkeletonCard v-for="item in 20" :key="item" class="inline-block card-wrap" />
    </div>
    <div v-else-if="!errorFavorites" class="mx-4 overflow-x-auto whitespace-nowrap">
      <Card v-for="item in favorites?.items" :key="item.name" :data="item" class="inline-block card-wrap" />
      <!-- {{ favorites.items }} -->
    </div>
  ...
  <q-dialog v-model="showDialogQR" position="bottom" full-width>
    <q-card class="bg-dark-500 h-min-[500px] px-4 pb-2 pt-4">
      ...
      <div class="text-grey">Quét QR Code này trên thiết bị khác để đăng nhập</div>
...
// import QrScanner from "qr-scanner"
  ...
  watch(qrRef, qrRef => {
    if (qrRef) {
      ...
      QRCode.toCanvas(qrRef, "anvs://login?token=" + compressToBase64 (JSON.stringify({
        token_name: authStore.token_name,
        token_value: authStore.token_value,
        user_data: authStore.user_data
      })))
