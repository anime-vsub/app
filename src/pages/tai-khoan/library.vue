<template>
  <q-item clickable @click="authStore.user ? gotoEditProfile() : (showDialogLogin = true)">
    <q-item-section avatar>
      <q-avatar size="55px">
        <img v-if="authStore.user?.avatar" :src="data.avatar">
    ...
    <q-card class="h-[60vh] bg-dark-500">
      ...
      <q-card-section>
        ...
        <div class="pt-4 relative">
          <input v-model="password" type="password" name="password" class="input" placeholder="Mật khẩu" />
        </div>
...
<script lang="ts" setup>
  ...
  const email = ref("thanh.30.10a9@gmail.com")
  const password = ref('Tachibanashingeso03')
