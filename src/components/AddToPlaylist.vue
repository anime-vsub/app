<template>
  <q-dialog>
    <q-card class="bg-dark-page min-w-[300px] max-h-[647px] max-w-[664px] px-6 rounded-xl">
      ...
      <q-card-section class="px-0 pt-0">
        <div v-if="playlists === undefined" class="flex pt-3 pb-9 items-center justify-center px-0">
        ...
        <q-list v-else class="px-0">
          <q-item v-for="item in playlists" :key="item.id" class="px-0">
            <q-item-section avatar class="min-w-0">
              <q-checkbox v-model="item.added" @update:model-value="$event ? emit('action:add', item.id) :
        ...
        <!-- creator -->
        <q-item v-if="!creatingPlaylist" class="mx-[-8px] mt-4 rounded-xl" clickable @click="creatingPlaylist = true">
          <q-item-section avatar class="min-w-0">
        ...
        <div v-else class="mt-4">
          <q-input v-model="nameNewPlaylist" bottom-slots label placeholder="Nhập theo danh sách phát..." counter
            maxlength="150" dense class="fix-input" stack-label :input-style="{
            'font-size': '14px',
            'padding-left': '0px'
          }" color="white" :rules="[val => !!val || 'Bắt buộc']" @keydown.stop @keypress.enter="createNewPlaylist">
          ...
          <q-card-actions align="right" class="pr-0 mr-[-8px] mt-2">
            <q-btn color="blue" rounded flat no-caps @click="creatingPlaylist = false">Hủy</q-btn>
            <q-btn color="blue" rounded flat no-caps @click="createNewPlaylist">Tạo</q-btn>
          </q-card-actions>
        </div>
      </q-card-section>
    </q-card>
...
<script lang="ts" setup>
  ...
  const showDialogAddToPlaylist = ref(true)
  ...
  const playlists = computedAsync(() => {
    ...
    return Promise.all(playlists.map(async item => {
      return {
        ...item,
  ...
  async function createNewPlaylist() {
    try {
      const {
        id
      } = await playlistStore.createPlaylist(nameNewPlaylist.value, false)
