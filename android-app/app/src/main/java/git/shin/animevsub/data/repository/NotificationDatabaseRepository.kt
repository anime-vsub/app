...
class NotificationDatabaseRepository @Inject constructor(
  ...
  suspend fun getCountNotify(): Result<DbNotificationCount?> = runCatching {
    ...
    val response = supabase.postgrest.rpc("get_count_notify", buildJsonObject {
      put("p_user_uid", uid)
    })
  ...
  suspend fun queryNotify(page: Int, pageSize: Int = 30): Result<List<DbNotificationItem>> =
    runCatching {
      val uid = getCurrentUid() ?: throw Exception("Not login")
      val response = supabase.postgrest.rpc("query_notify", buildJsonObject {
        put("p_page", page)
  ...
  suspend fun deleteNotify(
    ...
    val uid = getCurrentUid() ?: throw Exception("Not login")
    val response = supabase.postgrest.rpc("delete_notify", buildJsonObject {
      put("user_uid", uid)
      put("p_season", season)
      put("p_chapid", chapId)
    })
  ...
  suspend fun addNotify(item: NotificationItem): Result<Unit> = runCatching {
    val uid = getCurrentUid() ?: throw Exception("Not login")
    val response = supabase.postgrest.rpc("upsert_notify", buildJsonObject {
