...
open class WebViewAssetHandler: NSObject, WKURLSchemeHandler {
    ...
    open func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        if url.absoluteString.hasSuffix("#image") {
            var headers = urlSchemeTask.request.allHTTPHeaderFields ?? [:]
            headers.removeValue(forKey: "x-requested-with")
        
            processImageRequest(url: url, headers: headers, urlSchemeTask: urlSchemeTask)
        } else if url.absoluteString.hasSuffix("#resolve") {
            var headers = urlSchemeTask.request.allHTTPHeaderFields ?? [:]
            headers.removeValue(forKey: "x-requested-with")
        
            processResolveRequest(url: url, headers: headers, urlSchemeTask: urlSchemeTask)
        } else if url.absoluteString.hasSuffix("#animevsub-vsub_extra") {
            var headers = urlSchemeTask.request.allHTTPHeaderFields ?? [:]
            headers.removeValue(forKey: "x-requested-with")
        
            headers["referer"] = "https://animevietsub.tv"
            processAnimeVsubRequest(url: url, headers: headers, urlSchemeTask: urlSchemeTask)
        }
    ...
    private func processImageRequest(url: URL, headers: [String: String], urlSchemeTask: WKURLSchemeTask) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data, let response = response as? HTTPURLResponse {
                let mimeType = response.mimeType ?? "image/unknown"
                let webResourceResponse = URLResponse(
                    url: url,
                    mimeType: mimeType,
                    expectedContentLength: Int(response.expectedContentLength),
                    textEncodingName: "UTF-8"
                )
                urlSchemeTask.didReceive(webResourceResponse)
                urlSchemeTask.didReceive(data)
                urlSchemeTask.didFinish()
            } else {
                urlSchemeTask.didFailWithError(error ?? NSError(domain: "Network Error", code: 500, userInfo: nil))
            }
        }
        task.resume()
    }

    private func processResolveRequest(url: URL, headers: [String: String], urlSchemeTask: WKURLSchemeTask) {
        var request = URLRequest(url: url)
        for (key, value) in headers {
            request.setValue(value, forHTTPHeaderField: key)
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let response = response as? HTTPURLResponse {
                var resolvedUrl = url.absoluteString
                ...
                let webResourceResponse = URLResponse(
                    url: url,
                    mimeType: "text/plain",
                    expectedContentLength: resolvedUrl.utf8.count,
                    textEncodingName: "UTF-8"
                )
                var responseHeaders = ["x-location": resolvedUrl, "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET, POST, OPTIONS"]
                ...
                urlSchemeTask.didReceive(webResourceResponse)
                urlSchemeTask.didReceive(resolvedUrl.data(using: .utf8)!)
                urlSchemeTask.didFinish()
            } else {
                urlSchemeTask.didFailWithError(error ?? NSError(domain: "Network Error", code: 500, userInfo: nil))
            }
    ...
    private func processAnimeVsubRequest(url: URL, headers: [String: String], urlSchemeTask: WKURLSchemeTask) {
        var request = URLRequest(url: url)
        ...
        // Add Referer header
        request.setValue("https://animevietsub.tv", forHTTPHeaderField: "Referer")
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let response = response as? HTTPURLResponse {
                ...
                if let locationHeader = response.allHeaderFields["Location"] as? String {
                    ...
                    // If there's a redirect, make another request to the new URL
                    if let redirectUrl = URL(string: resolvedUrl) {
                        self.followRedirect(redirectUrl: redirectUrl, headers: headers, urlSchemeTask: urlSchemeTask)
                        return
                    }
                }
    ...
    private func followRedirect(redirectUrl: URL, headers: [String: String], urlSchemeTask: WKURLSchemeTask) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let response = response as? HTTPURLResponse {
                var resolvedUrl = redirectUrl.absoluteString
                if let locationHeader = response.allHeaderFields["Location"] as? String {
                    resolvedUrl = locationHeader
                }
