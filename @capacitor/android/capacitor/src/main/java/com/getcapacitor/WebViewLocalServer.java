...
public class WebViewLocalServer {
  ...
  WebViewLocalServer(Context context, Bridge bridge, JSInjector jsInjector, ArrayList<String> authorities,
      boolean html5mode) {
    ...
    this.segmentCache = new SegmentCache(context, 30 * 60_000);
  }
  ...
  public WebResourceResponse shouldInterceptRequest(WebResourceRequest request) {
    ...
    if (loadingUrl.toString().endsWith("#image")) {
      ...
      try {
        URL url = new URL(loadingUrl.toString());
...
//        String locationHeader = connection.getHeaderField("Location");
        ...
        responseHeaders.put("x-location", resolvedUrl);
    ...
    if (loadingUrl.toString().endsWith("#animevsub-vsub_extra")) {
      Map<String, String> headers = new HashMap<>(request.getRequestHeaders());
      headers.remove("x-requested-with");
      headers.put("referer", "https://animevietsub.tv");
      ...
      try {
        ...
        //
        responseHeaders.put("Access-Control-Allow-Origin", "*");
        responseHeaders.put("Access-Control-Allow-Methods", "PUT, GET, HEAD, POST, DELETE, OPTIONS");

        // responseHeaders.put("Location", resolvedUrl);
        responseHeaders.put("W-Location", resolvedUrl);
   ...
   * will be invoked
   * every time the <code>shouldInterceptRequest</code> method of the instance is
   * called with
